...
export default function UserTicketList() {
    ...
    const fetchTickets = async (branchId: string, isInitial = false) => {
    ...
    useEffect(() => {
        const init = async () => {
            ...
            setUser(session);
            fetchTickets(session.branchId, true);

            // Auto-poll user tickets every 5s
            const interval = setInterval(() => fetchTickets(session.branchId, false), 5000);
