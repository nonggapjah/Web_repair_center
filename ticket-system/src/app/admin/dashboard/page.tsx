                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(450px, 1fr))', gap: '2rem' }}>
                                ...
                                <div className="glass-panel" style={{ gridColumn: 'span 2', padding: '1.5rem', borderRadius: '24px' }}>
                                    <h3 style={{ marginBottom: '1.5rem' }}>📊 สรุปเคสที่เหลืออยู่</h3>
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
                                        {technicians.map(tech => {
                                            ...
                                            return (
                                                <div key={tech} style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                                    ...
                                                    <div style={{ flex: 1, height: '22px', background: '#f1f5f9', borderRadius: '11px', overflow: 'hidden' }}>
                                                        <div style={{ width: `${width}%`, height: '100%', background: 'linear-gradient(90deg, #7c3aed, #4f46e5)', borderRadius: '11px', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', paddingRight: '10px' }}>
                                                ...
                                                <div ref={provided.innerRef} {...provided.droppableProps} style={{ flex: 1, background: '#f1f5f9', borderRadius: '16px', padding: '0.8rem' }}>
                                                    {filteredTickets.filter(t => t.CurrentStatus === status).map((ticket, index) => (
                                                        <Draggable key={ticket.TicketID} draggableId={ticket.TicketID} index={index}>
                                                            {(provided) => (
                                                                <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} onClick={() => setSelectedTicket(ticket)} className="glass-panel" style={{ padding: '1rem', marginBottom: '0.8rem', background: '#fff', borderLeft: 'none', ...provided.draggableProps.style }}>
