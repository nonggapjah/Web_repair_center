                                ...
                                <div className="glass-panel" style={{ padding: '1.5rem', borderRadius: '24px' }}>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                                        {technicians.map(tech => {
                                            ...
                                            const pend = techAllTickets.filter(t => ['Open', 'Accepted'].includes(t.CurrentStatus));
                                            const proc = techAllTickets.filter(t => ['On Process', 'Repairing'].includes(t.CurrentStatus));
                                            const wait = techAllTickets.filter(t => t.CurrentStatus === 'Waiting Parts');
                                            const done = techAllTickets.filter(t => t.CurrentStatus === 'Completed' || t.CurrentStatus === 'Closed');
                                                ...
                                                <div key={tech} style={{ paddingBottom: '1rem', borderBottom: '1px solid #f1f5f9' }}>
                                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.8rem', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
                                                        ...
                                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                                                            <span title="รวม" style={{ background: '#f1f5f9', color: '#475569', padding: '0.2rem 0.5rem', borderRadius: '6px', fontSize: '0.7rem', fontWeight: '700' }}>📋 {techAllTickets.length}</span>
                                                            <span title="รอ" style={{ background: '#fef3c7', color: '#92400e', padding: '0.2rem 0.5rem', borderRadius: '6px', fontSize: '0.7rem', fontWeight: '700' }}>⏳ {pend.length}</span>
                                                            <span title="ทำ" style={{ background: '#dbeafe', color: '#1e40af', padding: '0.2rem 0.5rem', borderRadius: '6px', fontSize: '0.7rem', fontWeight: '700' }}>🛠️ {proc.length}</span>
                                                            <span title="อะไหล่" style={{ background: '#fee2e2', color: '#991b1b', padding: '0.2rem 0.5rem', borderRadius: '6px', fontSize: '0.7rem', fontWeight: '700' }}>📦 {wait.length}</span>
                                                            <span title="เสร็จ" style={{ background: '#dcfce7', color: '#166534', padding: '0.2rem 0.5rem', borderRadius: '6px', fontSize: '0.7rem', fontWeight: '700' }}>✅ {done.length}</span>
                                                        </div>
                                                    </div>

                                                    {(pend.length > 0 || proc.length > 0 || wait.length > 0) ? (
                                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                                            {[...pend, ...proc, ...wait].slice(0, 5).map(t => (
                                                                <div
                                                                    key={t.TicketID}
                                                                ...
                                                                >
                                                                    <div style={{ fontWeight: '600', maxWidth: '75%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
